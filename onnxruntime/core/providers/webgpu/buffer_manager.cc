...
namespace {
constexpr const char* METRICS_FILE = "webgpu_memory_metrics_no_optimization_fast_release.csv";
...
class BucketCacheManager : public IBufferCacheManager {
 ...
 protected:
  ...
  void UpdateMetrics(bool is_allocation, size_t buffer_size, bool called_from_destructor = false) {
    if (is_allocation) {
